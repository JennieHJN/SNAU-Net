...
import math
from net.pos_embed import get_2d_sincos_pos_embed
...
#     def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims_s=[64, 128, 256, 512],
#                  num_heads_s=[1, 2, 4, 8], mlp_ratios_s=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
#                  attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
#                  depths_s=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], num_stages=4, linear=False):
#         super().__init__()
#         self.num_classes = num_classes
#         self.depths = depths_s
#         self.num_stages = num_stages
#         self.num_patches = img_size//4
#         dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths_s))]  # stochastic depth decay rule
#         cur = 0
#
#         for i in range(num_stages):
#             # if i == 0:
#             #     patch_embed = Head(embed_dims_s[0])  #
#             # else:
#             #     patch_embed = PatchMerging(dim=embed_dims_s[i - 1],
#             #                                out_dim=embed_dims_s[i])
#             block = nn.ModuleList([SGBlock(
#                 dim=embed_dims_s[i], mask=True if (j%2==1 and i<num_stages-1) else False, num_heads=num_heads_s[i], mlp_ratio=mlp_ratios_s[i], qkv_bias=qkv_bias, qk_scale=qk_scale,
#                 drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer,
#                 sr_ratio=sr_ratios[i], linear=linear)
#                 for j in range(depths_s[i])])
#             norm = norm_layer(embed_dims_s[i])
#             cur += depths_s[i]
#
#             # setattr(self, f"patch_embed{i + 1}", patch_embed)
#             # setattr(self, f"block{i + 1}", block)
#             # setattr(self, f"norm{i + 1}", norm)
#         self.pos_embed = nn.Parameter(torch.zeros(1, self.num_patches*self.num_patches, embed_dims_s[0]))  # fixed sin-cos embedding
#
#         # classification head
#         self.head = nn.Linear(embed_dims_s[3], num_classes) if num_classes > 0 else nn.Identity()
#
#         self.apply(self._init_weights)
#
#     def _init_weights(self, m):
